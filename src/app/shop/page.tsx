    ...
    >
      ...
      <div className="flex flex-1 flex-col p-5">
        {!product.image && product.tag && (
          <span className="mb-2 inline-flex w-fit rounded-full bg-neutral-900 px-3 py-1 text-[11px] font-semibold uppercase tracking-wider text-white">
            {product.tag}
          </span>
        )}
  ...
  return (
    ...
    >
      <div className="flex-1">
        {product.tag && (
          <span className="mb-1.5 inline-flex rounded-full bg-neutral-900 px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider text-white">
            {product.tag}
          </span>
        )}
        <h3 className="text-sm font-semibold text-neutral-800 group-hover:text-primary transition-colors">
          {product.name}
        </h3>
      ...
      <ExternalLink className="h-4 w-4 shrink-0 text-neutral-300 transition-colors group-hover:text-primary" />
    </a>
  );
}
      ...
      <section className="bg-white pt-[var(--floating-nav-gap)] pb-6 md:pb-8">
        <div className="layout-page">
          <header className="mx-auto max-w-3xl px-4 text-center">
            <p className="text-sm font-normal lowercase tracking-normal text-neutral-500">
              karten-tricks.de
            </p>
            ...
            <div className="mt-8 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
              {SHOP_CATEGORIES.map((category) => (
                <a
                  key={category.id}
                  href={`#${category.id}`}
                  className="inline-flex h-10 items-center justify-center rounded-full bg-neutral-100 px-5 text-sm font-medium text-neutral-700 transition-colors hover:bg-neutral-900 hover:text-white"
                >
                  {category.label}
                </a>
              ))}
      ...
      <div id="shop-produkte" className="layout-page space-y-20 pb-20 pt-8">
        {SHOP_CATEGORIES.map((category) => (
          <section key={category.id} id={category.id} className="scroll-mt-28">
            <div className="mb-8">
              <h2 className="text-[clamp(1.5rem,4vw,2rem)] font-bold tracking-tight text-neutral-800">
                {category.label}
              </h2>
